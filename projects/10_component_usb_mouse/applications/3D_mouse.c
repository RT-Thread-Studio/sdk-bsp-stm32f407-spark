...
/* 鼠标响应时间 */
const static rt_uint32_t mouse_sample_times = 10;
...
struct icm_position
{
    ...
    volatile float x;           // 传感器 x 位置
    volatile float y;           // 传感器 y 位置
    volatile float z;           // 传感器 z 位置
    volatile rt_int8_t buff[4]; // 发送鼠标的值
};
...
static icm_position_t mouse_init_icm(void)
{
    ...
    /* 初始化传感器 icm20608 */
    icm_device->icm20608_device = icm20608_init("i2c2");
...
static void mouse_send_data(rt_device_t device)
{
    ...
    rt_device_write(device, HID_REPORT_ID_MOUSE, (const void*)icm_device->buff, 4);
	rt_thread_mdelay(5);
...
static void key_thread_entry(void *parameter)
{
    ...
    while (1)
    {
        ...
        if (mouse_key_2 | mouse_key_0)
        {
            ...
            {
                /* 获取鼠标键值，默认为 0 */
                icm_device->buff[0] = 0x08 | mouse_key_0 | mouse_key_2 << 1;
                if (mouse_key_2 & mouse_key_0)
                {
